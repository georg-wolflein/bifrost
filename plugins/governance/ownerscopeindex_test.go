...
// rate limits are bucketed and surfaced through their respective iterators.
func TestEachRateLimitForScopes_TokenAndRequest(t *testing.T) {
	gs := newOwnerScopeTestStore(t)
	gs.rateLimits.Store("rl-token", &configstoreTables.TableRateLimit{
		ID: "rl-token", VirtualKeyID: ownerStrPtr("vk1"),
		TokenMaxLimit: ownerI64Ptr(1000), TokenCurrentUsage: 900,
	})
	gs.rateLimits.Store("rl-req", &configstoreTables.TableRateLimit{
		ID: "rl-req", TeamID: ownerStrPtr("team1"),
		RequestMaxLimit: ownerI64Ptr(100), RequestCurrentUsage: 80,
	})

	t.Run("token limit only", func(t *testing.T) {
		ids := map[string]int64{}
		gs.EachRateLimitForScopes([]OwnerScope{{Type: "virtual_key", ID: "vk1"}, {Type: "team", ID: "team1"}},
	...
	t.Run("request limit only", func(t *testing.T) {
		ids := map[string]int64{}
		gs.EachRequestLimitForScopes([]OwnerScope{{Type: "virtual_key", ID: "vk1"}, {Type: "team", ID: "team1"}},
			func(id string, cur, max int64, st, sid string) { ids[id] = max })
		require.Len(t, ids, 1)
...
// BenchmarkEachBudget_FullScan measures the legacy full-store scan: every usage
// update visits every budget. Cost grows linearly with the budget count.
func BenchmarkEachBudget_FullScan(b *testing.B) {
	for _, n := range benchScaleSizes {
		...
		b.Run(fmt.Sprintf("budgets=%d", n), func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				var sum float64
				gs.EachBudget(func(_ string, currentUsage, _ float64, _, _ string) { sum += currentUsage })
				_ = sum
			}
		})
	}
