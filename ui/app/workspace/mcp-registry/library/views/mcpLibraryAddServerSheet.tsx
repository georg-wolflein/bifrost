		...
		<Sheet open={open} onOpenChange={(sheetOpen) => !sheetOpen && onClose()}>
			<SheetContent className="flex w-full flex-col overflow-x-hidden p-0 pt-4">
				<SheetHeader className="flex flex-col items-start px-0 py-4" headerClassName="mb-0 sticky px-8 -top-4 bg-card z-10">
					...
					<SheetDescription>
						This MCP server will be available org-wide for members to discover, install, and use.
					</SheetDescription>
				</SheetHeader>
					...
					<div className="flex-1 space-y-4 px-8 py-4">
						...
						<div className="grid grid-cols-2 gap-3">
							...
							<div className="space-y-2">
								...
								<Select value={connectionType} onValueChange={(v) => setValue("connection_type", v as MCPConnectionType)}>
									<SelectTrigger data-testid="mcp-add-connection-type">
										<SelectValue />
									</SelectTrigger>
									<SelectContent>
						...
						{isStdio ? (
							<div className="space-y-4 rounded-sm border p-4">
								...
								<div className="space-y-2">
									...
									<Input id="mcp-add-args" placeholder="comma separated, e.g. -y, my-package" data-testid="mcp-add-args-input" {...register("args")} />
								</div>
								<div className="space-y-2">
						...
						<div className="space-y-2">
							...
							<Select value={authType} onValueChange={(v) => setValue("auth_type", v as MCPAuthType)}>
								<SelectTrigger data-testid="mcp-add-auth-type">
									<SelectValue />
								</SelectTrigger>
								<SelectContent>
						...
						<div className="grid grid-cols-2 gap-3">
							<div className="space-y-2">
								<Label htmlFor="mcp-add-publisher">Publisher</Label>
								<Input id="mcp-add-publisher" placeholder="Acme Inc." data-testid="mcp-add-publisher-input" {...register("publisher")} />
							</div>
							<div className="space-y-2">
								<Label htmlFor="mcp-add-version">Version</Label>
								<Input id="mcp-add-version" placeholder="1.0.0" data-testid="mcp-add-version-input" {...register("version")} />
							</div>
						</div>
						<div className="grid grid-cols-2 gap-3">
							<div className="space-y-2">
						...
						<div className="space-y-2">
							...
							<Input id="mcp-add-tags" placeholder="comma separated, e.g. internal, database" data-testid="mcp-add-tags-input" {...register("tags")} />
						</div>
					</div>
